shared_examples "a protected admin controller" do |controller|
  ...
  describe "#index" do
    # controller名を指定していないので、複数のadmin controller specで使い回せる
    example "ログインフォームにリダイレクト" do
      # GETメソッドでindexアクションにアクセスするとログインフォームにリダイレクトされる
  ...
  describe "#show" do
    example "ログインフォームにリダイレクト" do
      # GETメソッドでshowアクションにアクセスするとログインフォームにリダイレクトされる
      # idは使われる前にリダイレクトされるので適当にid: 1としている
